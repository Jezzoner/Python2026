...
def validar(event):
   print("Se mando a llamar el metodo validar")
   usuario = usuario_caja_texto.get()
   password = password_caja_texto.get()
   print(f"Usuario: {usuario}, Password: {password}")
   #usuario = admin y password = administrador son los valores correctos
   if usuario == 'admin' and password == 'administrador':
      print('Login exitoso')
      showinfo(title='Login', message='Login Exitoso')
   elif usuario == '' or password == '':
      showwarning(title='Login', message='Debes proporcionar datos')
   else:
      print('Usuario y/o password incorrectos')
      showerror(title='Login', message='Usuario y/o password incorrectos')
...
login_boton.bind('<Return>', validar) #Presiona la tecla de enter
login_boton.bind('<Button-1>', validar) #click izq del mouse
